...
class Parser {
   ...
   *  tested
   */
  drive({type, path, interface:intf,snapshot='off'}) {
    switch (type) {
      case 'disk':
        return this.pushArg('-drive', `snapshot=${snapshot},file=${path},media=disk,if=${intf}`);

      case 'raw':
        return this.pushArg('-drive', `snapshot=${snapshot},file=${path},media=disk,if=${intf},format=raw`);

      case 'partition':
        return this.pushArg('-drive', `snapshot=${snapshot},file=${path},media=disk,if=${intf},cache=none`);

      case 'nbd':
        throw 'nbd unsupported';

      case 'cdrom':
        return this.pushArg('-drive', `snapshot=${snapshot},file=${path},media=cdrom,if=${intf}`);
    }
  }
      ...
      if (opts.hostToVmPortFwd || opts.vmToHostPortFwd) {
        ...
        if (opts.hostToVmPortFwd) {
          ext = opts.hostToVmPortFwd.reduce( (prev, fwd) => {
            return `${prev},hostfwd=tcp:${fwd.hostIp}:${fwd.hostPort}-${opts.ip}:${fwd.guestPort}`;
          }, ext);
        } // if
        ...
        if (opts.vmToHostPortFwd) {
          ext = opts.vmToHostPortFwd.reduce( (prev, fwd) => {
            return `${prev},guestfwd=tcp:${opts.ip}:${fwd.guestPort}-${fwd.hostIp}:${fwd.hostPort}`;
          }, ext);
        } // if
      } // if
