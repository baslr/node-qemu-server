...
class Parser {
  ...
  vmConfToArgs(conf) {
    ...
    // drives
    hw.drives.forEach(drive => this.drive(drive));
   ...
   *  tested
   */
  drive({ format='raw', media, path, if:intf, snapshot='off', copyOnRead, readonly, cache, aio, discard}) {
    let args = `snapshot=${snapshot},file=${path},media=${media},if=${intf}`;

    if (format)     args += `,format=${format}`;

    if (copyOnRead) args += `,copy-on-read=${copyOnRead}`;
    if (readonly)   args += `,readonly=${readonly}`;
    if (cache)      args += `,cache=${cache}`;
    if (aio)        args += `,aio=${aio}`;
    if (discard)    args += `,discard=${discard}`;
...
x if              ide|scsi|sd|mtd|floppy|pflash|virtio
x snapshot        on|off


x copy-on-read    on|off
x readonly        FLAG
x cache           none|writeback|unsafe|directsync|writethrough
x aio             threads|native
x discard         ignore|off|unmap|on // trim
*/

    return this.pushArg('-drive', args);
  }
  ...
  net(macAddr, card='rtl8139', mode='host', opts = {}) {
    if ('host' === mode || 'darwin' === osType) {
      ...
      if (opts.hostToVmPortFwd || opts.vmToHostPortFwd) {
        ext = `${ext},dhcpstart=${opts.ip}`;

        if (opts.hostToVmPortFwd) {
          ext = opts.hostToVmPortFwd.reduce( (prev, fwd) => {
            return `${prev},hostfwd=tcp:${fwd.hostIp}:${fwd.hostPort}-${opts.ip}:${fwd.vmPort}`;
          }, ext);
        } // if
        ...
        if (opts.vmToHostPortFwd) {
          ext = opts.vmToHostPortFwd.reduce( (prev, fwd) => {
            return `${prev},guestfwd=tcp:${opts.ip}:${fwd.vmPort}-${fwd.hostIp}:${fwd.hostPort}`;
          }, ext);
        } // if
      } // if
